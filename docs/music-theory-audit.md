...
- 所有理论课程难度至少应有 3 题；少于 3 题时即使有避重逻辑，也容易变成机械循环，不足以代表掌握。
...
- 反馈区只有用户实际填写或选择的字段才能标为“正确/应为”；系统补充的度数、半音、公式、协和分类等复盘信息应标为“参考”。
- 所有题卡都必须禁止空答案提交；空输入、未选择选项、未点亮节奏格时不能计入统计。
- 非选择题应至少提供一种键盘友好的符号/英文/数字等价写法；中文标准答案继续展示，但不能要求用户必须输入长中文句子才算对。
- 章节入口、难度卡、当前题目、输入框、提交/下一题/提示/重置、统计指标和节奏格必须有稳定 `data-testid`，保证真实浏览器回归测试可重复。
...
- 困难档“自然根音随机”必须同时填写并判定目标音、度数、半音数量、完整音程名和基础协和分类，避免学生只背音程名而没有完成完整分析。
- 困难档遇到 `B -> E#` 这类等音目标音时，反馈第一句必须说明 `E#` 与 `F` 同声音但不能直接改写，因为改写目标字母会改变音程度数。
- 反向拼写题也不是只写目标音；必须同时填写并判定目标音、度数、半音数量、完整音程名和基础协和分类，确保学生知道“为什么这样拼”。

## 和弦音构成

- 和弦音构成题训练的是 1、3、5、7、9 的结构顺序，不是按实际弹奏转位或音高从低到高排序。
- 三和弦、七和弦、九和弦的输入提示必须按当前题型动态显示对应顺序，避免三和弦题出现四个音的示例。
- D#、A# 等根音下可能产生 F##、C## 这类理论拼写；解释区应说明这是按字母骨架保留级数的结果，并桥接到实际谱面常用的 Eb、Bb 等音记法。
...
- 属九和弦 `G9` 的完整结构是 `G B D F A`，也就是属七 `G7` 加 9；不要把 9 理解成只在三和弦上加一个 A。
...
- `V7 - Imaj7` 是大调里最基本的属到主解决；综合题不能只考副属和弦这种特殊情况。
...
- `X...X...X...X...` 是每拍一个重音的基准 pattern，适合用来和错位重音对比。
