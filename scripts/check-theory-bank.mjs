...
const files = {
  ...
  naturalIntervalRecognition: readFileSync("src/utils/naturalIntervalRecognition.ts", "utf8"),
  ...
  homePage: readFileSync("src/components/HomePage.tsx", "utf8"),
  difficultySelect: readFileSync("src/components/DifficultySelect.tsx", "utf8"),
  intervalDifficultySelect: readFileSync("src/components/IntervalDifficultySelect.tsx", "utf8"),
  naturalIntervalTrainer: readFileSync("src/components/NaturalIntervalTrainer.tsx", "utf8"),
  theoryDifficultySelect: readFileSync("src/components/TheoryDifficultySelect.tsx", "utf8"),
  ...
  passScopePanel: readFileSync("src/components/PassScopePanel.tsx", "utf8"),
...
const requiredSnippets = [
  ...
  ["requiresTargetInput", "反向拼写题应显示目标音输入框，自然根音随机识别题不应重复填写已知目标音"],
  ["度数（已知）", "反向拼写题应把题面已知的度数展示为只读信息"],
  ["完整音程名（已知）", "反向拼写题应把题面已知的完整音程名展示为只读信息"],
  ["data-testid=\"blind-listening-toggle\"", "音程识别题应支持盲听模式，避免只看题面音名做听感判断"],
  ["data-testid=\"blind-listening-notice\"", "盲听模式开启后应明确提示目标音先隐藏"],
  ["data-testid=\"blind-listening-reveal\"", "盲听模式提交后应揭示目标音和完整题面"],
  ["目标音先隐藏", "盲听模式文案需要说明隐藏目标音的学习目的"],
  ["`${question.root} -> ?`", "盲听模式题面应隐藏目标音，只保留根音和问号"],
  ...
  ["b9 clash", "自由输入题应有键盘友好的英文/符号可接受写法"],
  ["data-testid=\"submit-answer-button\"", "题卡提交按钮需要稳定测试定位"],
  ...
  ["错题复习中心", "练习页应有集中错题复习入口"],
  ["data-testid=\"review-practice-panel\"", "错题复习中心需要稳定测试定位"],
  ...
  ["data-testid=\"rhythm-mobile-beat-grid\"", "节奏 16 格应按 4 拍分组显示，移动端不应依赖横向拖动"],
  ["data-testid={`rhythm-beat-group-${beatIndex + 1}`}", "节奏网格每拍分组需要稳定测试定位"],
  ["手机上按 4 拍折行", "节奏网格说明应明确移动端按拍折行"],
  ["1 e & a", "节奏网格每拍应显示 1 e & a 的十六分口令"],
  ...
  ["阶段通关标准", "统计模块应展示通关条件，避免把做题数量误当作掌握"],
  ["当前题型的纸面推导、规则识别和输入表达", "通关说明应明确当前考核不能证明真实演奏、听辨和歌曲应用"],
  ["data-testid=\"achievement-panel\"", "统计模块应加入成就反馈，而不是只显示冷冰冰的数字"],
  ["data-testid=\"achievement-badge\"", "成就徽章需要稳定测试定位"],
  ["data-testid=\"next-achievement-hint\"", "成就反馈应提示下一枚徽章差什么"],
  ["练习徽章", "统计面板应显示练习徽章区"],
  ["连对 10", "成就反馈应覆盖最高连对里程碑"],
  ["错题清零", "成就反馈应鼓励清掉当前待回炉错题"],
  ["速算手感", "成就反馈应连接到平均用时，而不只看题量"],
  ["getReviewQuestionLabels(stats)", "错题清零徽章应基于当前待回炉题，而不是累计错题次数"],
  ...
  ["data-testid=\"pass-scope-panel\"", "每个难度选择页应说明通关能证明与不能证明什么"],
  ["通关后能证明", "课程目标显示应明确当前题型通过后的证据范围"],
  ["还不能证明", "课程目标显示应明确纸面答题不能证明真实演奏、闭眼听辨或歌曲应用"],
  ["下一步练法", "课程目标显示应给出从规则题迁移到指板、耳朵、节拍或歌曲的下一步"],
  ["你已经能闭眼听辨所有音程", "音程课程必须明确普通通关不等于闭眼听辨能力"],
  ["你已经会在吉他上按出好听 voicing", "和弦构成课程必须明确纸面和弦音不等于真实 voicing 应用"],
  ["你已经拥有稳定 groove", "节奏课程必须明确网格题不等于真实律动稳定"],
  ["<PassScopePanel scopeId={CHORD_CHAPTER.id}", "和弦难度页应接入通关证据边界"],
  ["<PassScopePanel scopeId={INTERVAL_CHAPTER.id}", "音程难度页应接入通关证据边界"],
  ["<PassScopePanel scopeId={chapter.id}", "理论课程难度页应接入通关证据边界"],
];
